...
Command: npx gltfjsx@6.2.14 public/models/Pigeon.gltf -o src/components/Pigeon.jsx -r public 
*/

import { useRef, useEffect } from 'react';
import { useGLTF, useAnimations } from '@react-three/drei';

export default function Pigeon({ hovered, ...props }) {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF('/models/Pigeon.gltf');
  const { actions } = useAnimations(animations, group);
  console.log('actions: ', actions);
  useEffect(() => {
    const anim = hovered ? 'Death' : 'Fast_Flying';
    console.log('actions[anim]', actions, anim, actions[anim]);
